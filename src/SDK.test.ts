    ...
    describe("fetch", () => {
        it("should return data fetched from the api", async () => {
            ...
            vi.spyOn(headers, "buildAuthedHeaders")
                .mockImplementation((token: string) => ({ Authorization: `Bearer ${token}` } as any));
            const sdk = new SDK();
            await sdk.init();
        ...
        it("should relaunch authentication flow on authentication error received from the api", async () => {
            const payload = { field: "value" };
            const authenticateStub = vi.spyOn(SDK.prototype as any, "authenticate").mockImplementation(() => {});
            vi.stubGlobal("fetch", vi.fn()
                .mockImplementationOnce(() => buildUnauthorizedResponse())
                .mockImplementationOnce(() => buildApiResponse(payload))
            );
            vi.spyOn(headers, "buildAuthedHeaders").mockImplementation(() => ({} as any));
            const sdk = new SDK();
